...
//****************
  Function SD_Fisc(wCod,wDat,wAlias,wCent,wPendenc,wTransit,wProgram)     // Saldo Atualizado do Fiscal
//****************
 db_use    := Alias()
 ord_index := INDEXORD()          // Ultimo indice utilizado
 wrecnoex  := Recno()
 ...
 If XY_ESTOQODOO = "S"
    ...
    If wProgram <> "pfile"
       wCent:=Cons_Saldo_oDoo(wCod,Pid,xy_cgc)
    Else
       wCent:=SaldooDooGeral(wCod,Pid)  // Consulta Saldos no oDoo todos os CNPJ
    Endif

    @ 23,61 Say wCent pict "99999999"
    Setcolor(cor)
    Inkey(1)
...
//***********************
  Function SaldooDooGeral(wCod,Pid)      // Consulta Saldos no oDoo todos os CNPJ
//***********************
  Sele 58
  Veriout(SETCOLOR(),23,"filiais")
  ...
  wRet:=EstoqueOdooGeral(w_arq)    // Busca saldo de todos CNPJ

  RestScreen(05,01,23,78,telsd)
  ...
  While M_Ct <= Val(wRet)-1
    ...
    If m_Ct = 1
       wRegional:=DevolveRegional(I_Fili,@listFil)
    Endif
...
//*********************
  Function EstoqueOdooGeral(cArquivoCSV)    // Busca saldo de todos CNPJ
//*********************
Local nPointer := 0
