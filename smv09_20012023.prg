 ...
 *Verifcx(m_dat)              && Aceita o F7 se inibir essa opcao
  ...
  PRIV M_QTD[30],M_COD[30],M_PRD[30],M_STO[30],M_PRE[30],M_COM[30],M_AUTOR[30],w_garant[30],m_grfator[30],m_vezesci[30],wnSerie[30]
  ...
  */
  ...
  While M_CONT = "S"
    ...
    If LASTKEY() = 27
     ...
     Endif
      ...
      ElseIf wDescontoFilialSup = .F.
         If Loja = 9 .and. m_datsi >= ctod("23/01/2023") .and. m_datsi <= ctod("28/02/2023")
      ...
      If wSemIdentificacao = .T.      // Nao Permitir outros Deptos sem cadastro de cliente
         If m_grupo <> 6 .and. m_grupo <> 18 .and. m_grupo <> 19
     ...
     m_said[&M] := PegaSaidaItem(@wOpcao,@M_NRCD,wTentantiva,I_FilOrigem,.t.)  // Funcoes.prg
     OldOpcao = wOpcao

     wCont = .f.
     If (wOpcao = 2 .or. wOpcao = 3 .or. wOpcao = 4) .and. (M_NRCD = 10 .or. M_NRCD = 11 .or. M_NRCD = 19 .or. M_NRCD = 20 .or. M_NRCD = 99 .or. M_NRCD = 44)
        wCont = .t.
     Endif

     If wOpcao = 4 .and. M_NRCD = 44
        wCont = .f.
     Endif
     ...
     *wBuscaEstoque36 = .f.
     ...
     If M_cod[&m] >5 .and. Trim(m_said[&M]) = "LOJA/MOST"
        M_Res := Ret_Res(m_cod[&m])    // Funcoes.prg
        Stor 0 to Cent,m_pendenc,wTransit

        @ 19,15 Say "Aguarde, checando estoque... "
        SD_Fisc(m_cod[&m],,,@Cent,@M_Pendenc,@wTransit)
        M_SdPendenc := SD_Pend(m_cod[&m])
    ...
    *If Trim(m_said[&M]) = "CD" .or. wBuscaEstoque36 = .t.
     If Trim(m_said[&M]) = "CD"
        ...
        If !Empty(m_said[&M]) .and. wCont = .t.
           ...
           Endif
           If wSt = "Link Fora"
              wSt = "Ok"        // Reserva Manual
           Endif
           If wSt <> "Ok"
              @ 19,15 say "Produto Sem Saldo no CD: "+Strzero(wConsultaCD,2)
              inkey(2)
              @ 19,15 clear to 19,50
              If wOpcao <> 2
              ...
              If I_Priorid = M_NRCD
                 ...
                 m_said[&M] := PegaSaidaItem(@wOpcao,@M_NRCD,wTentantiva,I_FilOrigem,.f.)     // Funcoes.prg
                 ...
                 Endif
                 If wSt <> "Ok"
                    @ 19,15 say "Produto Sem Saldo no CD: "+Strzero(M_NRCD,2)
                    inkey(2)
                    @ 19,15 clear to 19,50
                    Stor 0 to M_NRCD
                    Stor 3 to wOpcao
                    ...
                    If wSt <> "Ok"
                       ...
                       If wSt <> "Ok"
                          ...
                          Loop
                       Else
                          @ 19,15 say "Produto Com Saldo no CD: "+Strzero(M_NRCD,2)
                          inkey(1)
                       Endif
                    Else
                       @ 19,15 say "Produto Com Saldo no CD: "+Strzero(M_NRCD,2)
              ...
              Endif
           Else
              @ 19,15 say "Produto Com Saldo no CD: "+Strzero(wConsultaCD,2)
              inkey(1)
           Endif
        Endif
        restscreen(16,08,21,73,telprior)
    ...
    Release mfrt
     ...
     If xy_cap_int = "IN" .and. m_datsi >= ctod("01/01/2023") .and. m_datsi >= ctod("31/03/2023")
     ...
     If xy_cap_int = "IN" .and. m_datsi >= ctod("01/12/2021") .and. m_datsi <= ctod("31/03/2023")
     ...
     If w_garant[&m] > 0
        ...
        If (m_tipo = 2 .or. m_tipo = 3 .or. m_tipo = 6) .and. w_garant[&m] > 0 .and. m_vezes[&m] <= 3
           m_tx_gr = 0
        ElseIf m_tipo = 5 .and. w_garant[&m] > 0 .and. m_vezes[&m] <= 5
           m_tx_cc = 0
           m_tx_gr = 0
        ElseIf m_tipo = 5 .and. w_garant[&m] > 0 .and. m_vezes[&m] > 5
        ...
        // Utilizando tabela de garantia da Capital sem Juros
        If XY_Cap_Int = "CC" .and. M_Datsi >= ctod("01/07/2022") .and. M_Datsi <= ctod("01/07/2030")
           If w_vezesgr[&m] <= 10
              m_tx_cc = 0
              m_tx_gr = 0
           Endif
...
*/
 ...
 While M_CT <= VAL(m) - 1
   ...
   Repl CD_CODIOR WITH M_NOTA,CD_DATAO WITH M_ULTIMA,CD_CODICL WITH M_CLIE,CD_NOMECL WITH M_NOME,CD_CGF WITH M_CGF,CD_NSERIE WITH wnSerie[M_CT],;
        ...
        CD_VENDED WITH V_NOME,CD_CODVEN WITH M_VEND,CD_NUMPRE WITH M_NUM,CD_TIPOV WITH M_TIPO,CD_VEZES WITH M_VEZES[M_CT], ;
        CD_ENTRA WITH M_ENTRAD,CD_NUM1 WITH M_NUM1,CD_NUM2 WITH M_NUM2,CD_NUM3 WITH M_NUM3,CD_NUM4 WITH M_NUM4,CD_FONE WITH Trim(FONECEL), ;
