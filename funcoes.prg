...
//****************
  Function SD_Fisc(wCod,wDat,wAlias,Centx,wPendenc,wTransit,wProgram)     // Saldo Atualizado do Fiscal
//****************
 db_use    := Alias()
 ord_index := INDEXORD()          // Ultimo indice utilizado
 wrecnoex  := Recno()
 ...
 If XY_ESTOQODOO = "S"
    cor=Setcolor()
    Setcolor("g+/b")
    telsd=savescreen(23,01,23,78)
    @ 23,01 clear to 23,78
    @ 23,25 say "Aguarde, Consultando Saldo no oDoo !"

    If wProgram <> "pfile"
       Centx:=Cons_Saldo_oDoo(wCod,Pid,xy_cgc)
    Else
       SaldooDooGeral(wCod,Pid,@Centx)  // Consulta Saldos no oDoo todos os CNPJ
    Endif

    @ 23,61 Say Centx pict "99999999"
    Setcolor(cor)
    Inkey(1)
    Restscreen(23,01,23,78,telsd)
    Release telsd
    Stor 0 to wTransit,wPendenc
    Return
 Endif
...
//******************
  Function TaxaPromo(wVezesPromo,i_tx_cc,TemTaxa,m_grupo,m_tipo)
//******************
  // Joaozinho
  // Taxas promocionais com tempo limitado
   If M_Datsi >= I_DataIniCC_01 .and. M_Datsi <= I_DataFimCC_01 .and. m_grupo = 1 .and. m_tipo = 5
      If wVezesPromo <= I_VezesCC_01
         i_tx_cc = I_TaxaCC_01
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCC_02 .and. M_Datsi <= I_DataFimCC_02 .and. m_grupo = 2 .and. m_tipo = 5
      If wVezesPromo <= I_VezesCC_02
         i_tx_cc = I_TaxaCC_02
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCC_03 .and. M_Datsi <= I_DataFimCC_03 .and. m_grupo = 3 .and. m_tipo = 5
      If wVezesPromo <= I_VezesCC_03
         i_tx_cc = I_TaxaCC_03
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCC_04 .and. M_Datsi <= I_DataFimCC_04 .and. m_grupo = 4 .and. m_tipo = 5
      If wVezesPromo <= I_VezesCC_04
         i_tx_cc = I_TaxaCC_04
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCC_06 .and. M_Datsi <= I_DataFimCC_06 .and. m_grupo = 6 .and. m_tipo = 5
      If wVezesPromo <= I_VezesCC_06
         i_tx_cc = I_TaxaCC_06
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCC_07 .and. M_Datsi <= I_DataFimCC_07 .and. m_grupo = 7 .and. m_tipo = 5
      If wVezesPromo <= I_VezesCC_07
         i_tx_cc = I_TaxaCC_07
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCC_08 .and. M_Datsi <= I_DataFimCC_08 .and. m_grupo = 8 .and. m_tipo = 5
      If wVezesPromo <= I_VezesCC_08
         i_tx_cc = I_TaxaCC_08
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCC_16 .and. M_Datsi <= I_DataFimCC_16 .and. m_grupo = 16 .and. m_tipo = 5
      If wVezesPromo <= I_VezesCC_16
         i_tx_cc = I_TaxaCC_16
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCC_17 .and. M_Datsi <= I_DataFimCC_17 .and. m_grupo = 17 .and. m_tipo = 5
      If wVezesPromo <= I_VezesCC_17
         i_tx_cc = I_TaxaCC_17
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCC_18 .and. M_Datsi <= I_DataFimCC_18 .and. m_grupo = 18 .and. m_tipo = 5
      If wVezesPromo <= I_VezesCC_18
         i_tx_cc = I_TaxaCC_18
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCC_19 .and. M_Datsi <= I_DataFimCC_19 .and. m_grupo = 19 .and. m_tipo = 5
      If wVezesPromo <= I_VezesCC_19
         i_tx_cc = I_TaxaCC_19
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCa_01 .and. M_Datsi <= I_DataFimCa_01 .and. m_grupo = 1 .and. m_tipo = 3
      If wVezesPromo <= I_VezesCa_01
         i_tx_cc = I_TaxaCa_01
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCa_02 .and. M_Datsi <= I_DataFimCa_02 .and. m_grupo = 2 .and. m_tipo = 3
      If wVezesPromo <= I_VezesCa_02
         i_tx_cc = I_TaxaCa_02
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCa_03 .and. M_Datsi <= I_DataFimCa_03 .and. m_grupo = 3 .and. m_tipo = 3
      If wVezesPromo <= I_VezesCa_03
         i_tx_cc = I_TaxaCa_03
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCa_04 .and. M_Datsi <= I_DataFimCa_04 .and. m_grupo = 4 .and. m_tipo = 3
      If wVezesPromo <= I_VezesCa_04
         i_tx_cc = I_TaxaCa_04
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCa_06 .and. M_Datsi <= I_DataFimCa_06 .and. m_grupo = 6 .and. m_tipo = 3
      If wVezesPromo <= I_VezesCa_06
         i_tx_cc = I_TaxaCa_06
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCa_07 .and. M_Datsi <= I_DataFimCa_07 .and. m_grupo = 7 .and. m_tipo = 3
      If wVezesPromo <= I_VezesCa_07
         i_tx_cc = I_TaxaCa_07
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCa_08 .and. M_Datsi <= I_DataFimCa_08 .and. m_grupo = 8 .and. m_tipo = 3
      If wVezesPromo <= I_VezesCa_08
         i_tx_cc = I_TaxaCa_08
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCa_16 .and. M_Datsi <= I_DataFimCa_16 .and. m_grupo = 16 .and. m_tipo = 3
      If wVezesPromo <= I_VezesCa_16
         i_tx_cc = I_TaxaCa_16
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCa_17 .and. M_Datsi <= I_DataFimCa_17 .and. m_grupo = 17 .and. m_tipo = 3
      If wVezesPromo <= I_VezesCa_17
         i_tx_cc = I_TaxaCa_17
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCa_18 .and. M_Datsi <= I_DataFimCa_18 .and. m_grupo = 18 .and. m_tipo = 3
      If wVezesPromo <= I_VezesCa_18
         i_tx_cc = I_TaxaCa_18
         TemTaxa = .t.
      Endif
   ElseIf M_Datsi >= I_DataIniCa_19 .and. M_Datsi <= I_DataFimCa_19 .and. m_grupo = 19 .and. m_tipo = 3
      If wVezesPromo <= I_VezesCa_19
         i_tx_cc = I_TaxaCa_19
         TemTaxa = .t.
      Endif
   Endif
...
//***********************
  Function RetAvistaSuper(wDataIni_1,wDataFim_1,wDataIni_2,wDataFim_2,wDataIni_3,wDataFim_3,wDataIni_4,wDataFim_4,wAreaSuperv1,wAreaSuperv2,wAreaSuperv3,wAreaSuperv4,I_DESCAVEL,I_DESCAVMO,I_DCELECAP,I_DCELEINT,I_DCMOVINT,I_DCCARNE,I_DCMOVCAP,I_DCCHEQUE,wDescontoFilialSup)
//***********************
  Local wPass1,wPass2,wPass3,wPass4
  Stor "n" to wPass1,wPass2,wPass3,wPass4

  // Promocao de 5% para as Lojas por Supervisor Joaozinho em 23/08/2022

  If M_Datsi >= wDataIni_1 .and. M_Datsi <= wDataFim_1
     wPass1 = "s"
  Endif
  If M_Datsi >= wDataIni_2 .and. M_Datsi <= wDataFim_2
     wPass2 = "s"
  Endif
  If M_Datsi >= wDataIni_3 .and. M_Datsi <= wDataFim_3
     wPass3 = "s"
  Endif
  If M_Datsi >= wDataIni_4 .and. M_Datsi <= wDataFim_4
     wPass4 = "s"
  Endif
  If wPass1 = "s" .or. wPass2 = "s" .or. wPass3 = "s" .or. wPass4 = "s"
     Sele 24
     Veriout(SETCOLOR(),23,"filiais")
     Go Top
     While !Eof()
       If Fili = I_Fili .and. Area > 0
          If wPass1 = "s" .and. Area = wAreaSuperv1
             wDescontoFilialSup = .t.
          Endif
          If wPass2 = "s" .and. Area = wAreaSuperv2
             wDescontoFilialSup = .t.
          Endif
          If wPass3 = "s" .and. Area = wAreaSuperv3
             wDescontoFilialSup = .t.
          Endif
          If wPass4 = "s" .and. Area = wAreaSuperv4
             wDescontoFilialSup = .t.
          Endif
       Endif
       Skip
...
//********************
  Function RetornacampoDBF(wField,wDbf)
//********************
  Sele &wDbf
  wRet := isfield(wField)
Return(wRet)
...
//*****************
  PROCEDURE RetnSerie(wCodigo)
//*****************
 Local i,telaimei,cont,wContador
 wInicionSerie = ""

 Priv lista :={},nSerieLista[99999]
 i = "00001"
 wContador =0
 Sele 44
 Set Order to 2
    ...
    If Vendido_lj = 0
       ...
       If wImp = "s"
          If !Empty(cel_nSerie)
             wContador++
          Endif
          aadd(lista,Str(field->cel_codi,6)+" "+(cel_nSerie)+"     "+wBipado+"   "+Str(orcamento)+"       "+wMotivo+"        "+wTransf+"         "+wRoubado)
          nSerieLista[&i] = cel_nSerie
          i = Str(Val(i)+1)
       Endif
    Endif
 ...
 *wait len(lista)
 ...
 While cont = .t.
  If wContador = 0
     ...
     Exit
  Endif
  telanSerie=savescreen(12,01,23,78)
  SETCOLOR("w+/b")
  @ 12,01 Say "Codigo N§ S‚rie               Bipado    Or‡am   Mostru   Transfer   Roubado"
  SETCOLOR("g/b")
  @ 13,01 to 13,78
  m_opca = achoice(14,01,22,77,lista,.t.,,)
  Setcolor(cor)
  restscreen(12,01,23,78,telanSerie)
  If Lastkey() = 4 .or. Lastkey() = 19
     wnSerie[&M] = ""
  Endif
  If Val(i) >1
     If lastkey() <> 27
        wnSerie[&M] := nSerieLista[M_OPCA]
        cont = .f.
     Else
        wnSerie[&M] = ""
     Endif
  Else
     wnSerie[&M] = ""        // Nova Verificacao pelo indice caso a anterior falhe
     Seek Str(wCodigo,6,0)
     While !Eof() .and. cel_codi = wCodigo
       If Vendido_lj = 0
          ...
          Endif
          If wImp = "s"
             aadd(lista,Str(field->cel_codi,6)+" "+(cel_nSerie)+"     "+bipado+"   "+Str(orcamento)+"         "+wMotivo+"             "+wTransf+"             "+wRoubado)
             nSerieLista[&i] = cel_nSerie
             i = Str(Val(i)+1)
          Endif
          Exit
...
//********************
  Function ValidarnSerie(wCodCel,wnSerie)      // Retorna Nr de Serie
//********************
 Local wRet,wBusca
 wRet = .t.
 ...
 If !Eof()
    IF orcamento >0
       Alert("N§ S‚rie Indisponivel. Produto est  em Or‡amento !")
       wRet = .f.
    Endif
    IF Bipado = "T"
       Alert("N§ S‚rie Indisponivel. Produto est  em Transito !")
       wRet = .f.
    Endif
    IF cel_transf >0
       Alert("Esse N§ S‚rie Foi Transferido! "+trim(cel_para)+" "+dtoc(cel_datatr))
       wRet = .f.
    Endif
    If(At("Roubado",Motivo)) >0
       Alert("N§ S‚rie N„o Est  Disponivel, Motivo : Roubo/Furto")
       wRet = .f.
    Endif
    IF !Empty(devolucao)
       Alert("Produto Devolvido em "+dtoc(devolucao))
       wRet = .f.
    Endif
    IF !Empty(cel_datas)
       Alert("Produto Vendido em "+dtoc(cel_datas))
       wRet = .f.
    Endif
    IF !Empty(cel_para_2)
       Alert("Produto Transferido p/ "+trim(cel_para_2))
       wRet = .f.
    Endif
    IF !Empty(cel_para_3)
       Alert("Produto Transferido p/ "+trim(cel_para_3))
       wRet = .f.
    Endif
    IF !Empty(cel_para_4)
       Alert("Produto Transferido p/ "+trim(cel_para_4))
       wRet = .f.
    Endif
    IF !Empty(cel_para_5)
       Alert("Produto Transferido p/ "+trim(cel_para_5))
       wRet = .f.
    Endif
    IF Empty(bipado)
       Alert("N§ S‚rie N„o Conferido; Favor Bipar a Entrada desse Produto")
       wRet = .f.
    Endif
 Else
    Alert("N§ S‚rie N„o Encontrado para Este Codigo ! ")
    wRet = .f.
 Endif
 If !Empty(wnSerie)
    m_imps = "s"
    ...
    */
    ...
    Endif
    */
    If Trim(wnSerie)="000000000000000"
       m_imps = "n"
    ElseIf Trim(wnSerie)="111111111111111"
       m_imps = "n"
    ElseIf Trim(wnSerie)="222222222222222"
       m_imps = "n"
    ElseIf Trim(wnSerie)="333333333333333"
       m_imps = "n"
    ElseIf Trim(wnSerie)="444444444444444"
       m_imps = "n"
    ElseIf Trim(wnSerie)="555555555555555"
       m_imps = "n"
    ElseIf Trim(wnSerie)="666666666666666"
       m_imps = "n"
    ElseIf Trim(wnSerie)="777777777777777"
       m_imps = "n"
    ElseIf Trim(wnSerie)="888888888888888"
       m_imps = "n"
    ElseIf Trim(wnSerie)="AAAAAAAAAAAAAAA"
       m_imps = "n"
    ElseIf Trim(wnSerie)="BBBBBBBBBBBBBBB"
       m_imps = "n"
    ElseIf Trim(wnSerie)="CCCCCCCCCCCCCCC"
       m_imps = "n"
    ElseIf Trim(wnSerie)="DDDDDDDDDDDDDDD"
       m_imps = "n"
    ElseIf Trim(wnSerie)="EEEEEEEEEEEEEEE"
       m_imps = "n"
    ElseIf Trim(wnSerie)="FFFFFFFFFFFFFFF"
       m_imps = "n"
    ElseIf Trim(wnSerie)="GGGGGGGGGGGGGGG"
       m_imps = "n"
    ElseIf Trim(wnSerie)="HHHHHHHHHHHHHHH"
       m_imps = "n"
    ElseIf Trim(wnSerie)="XXXXXXXXXXXXXXX"
       m_imps = "n"
    ElseIf Trim(wnSerie)="yyyyyyyyyyyyyyy"
       m_imps = "n"
    ElseIf Trim(wnSerie)="ZZZZZZZZZZZZZZZ"
       m_imps = "n"
    Endif
    If m_imps = "n"
       Alert("N§ S‚rie N„o Permitido ! ")
       wRet = .f.
    Endif
 Endif
...
//*********************
  Function ZeraOrcaIMei(wIMei,wnSerie,wTipo)      // Zerando nr de Orcamento em IMei
//*********************
 Local wRet
 wRet = .f.
 Sele 44
...
Read
If Empty(wProd)
   RestScreen(02,01,24,79,ssTela)
   Return(0)
Endif
@ 02,01 clear to 22,79
@ 02,01 to 02,79
SETCOLOR("w+/b")
@ 03,02 Say "Descri‡„o do Produto           Codigo Cor/Marca      A Vista  Loja  CD   CDZ's"
SETCOLOR("g/b")
@ 04,01 to 20,79
@ 11,20 SAY "Verificando Estoques Disponiveis..."

wProd = alltrim(wProd)
Stor 0 to wTotCd,wTotLJ,wTotCDs
m = "00001"
 ...
 *wTotal = (wTotx+wSaldoLj) - (wAvar+wAutor)
  ...
  wTotCd = wTotCd + wSaldoCD
  wTotLJ = wTotLJ + wSaldoLJ
  wTotCDs=wTotCDs + wTotal

  Sele 3
  Set Order to 2
  If wTotal >0 .or. wSaldoLj >0
     aadd(lista,Substr(field->cd_prod,1,30)+Str(cd_codi)+"  "+Substr(cd_cor,1,6)+"/"+Substr(cd_fabr,1,6)+""+Str(cd_avista2,9,2)+""+Str(wSaldoLJ,5)+""+""+Str(wSaldoCD,5)+" "+Str(wTotal,5))
     wCod[&m] = cd_codi
     M = Str(Val(M)+1)
  Endif
Enddo
...
@ 21,63 Say wTotLJ  pict "9999"
@ 21,68 Say wTotCd  pict "99999"
@ 21,74 Say wTotCDs pict "99999"

m_opca = achoice(05,02,19,78,lista,.t.,,)
setcolor(cor)
RestScreen(02,01,24,79,ssTela)
If Lastkey() = 4 .or. Lastkey() = 19
   Return(0)
...
If Val(M) >1
   ...
   Endif
Endif
Release wSaldoLj,wTotal,wTotCd,wTotLJ,wTotCDs,M
Release wTotx,wAvar,wAutor,m_opca,wProd
Return(0)
...
//***************
  ...
  If !Eof() .and. Empty(ba_partic)
     Alert(Trim(ba_nome)+", Por Favor Digite seu Nome Completo")
     wPs = "n"
     While .t.
       wCpf = space(14)
       wNome= space(50)
...
//****************
  Function Ret_StFone(wFone,FixoCel)   // Retorna Status do Numero de Telefone
//****************
  Local wRet
  wRet = .t.
  If !Empty(wFone)
...
//******************
  PROC PegaSaidaItem(wOpcao,NRCD,wTentatita,wFilOrigem,wCnpJ,wRegional)   // Retorna a saida do produto conforme sua regional
//******************
 Local sdp
 M_SAIDA = ""
 M_ROTAX = 0
 While .t.
    ...
    ElseIf wOpcao = 2                   // Entregar na regional
       ...
       ElseIf I_Priorid = 19
          ...
          M_NRCD  = 19
       ElseIf I_Priorid = 20
          *If M_Datsi > ctod("30/11/2022") .and. wFilOrigem = "13"   // Alteracao sugerida por Edione em 29/11/2022
          *   M_SAIDA = "CD IGUATU/03"
          *Else
              M_SAIDA = "CD IGUATU"
          *Endif
          M_NRCD  = 20
    ...
    Else
       ...
       */
       ...
       SETCOLOR(M_CORA)
       Use
       If M_NRCD = 01 .or. M_NRCD = 12 .or. M_NRCD = 18 .or. M_NRCD = 22 .or. M_NRCD = 33 .or. M_NRCD = 44 .or. M_NRCD = 62
...
//*************************
  Procedure InsereOrcamento(wOrcamento,wCpfVendedor,wSaidaCds,SaidaCDSN,wFilial)
//*************************
  Alert("Opera‡„o Suspensa, Envio pelo Credi ou Caixa !")
  Return
 ...
 While !Eof() .and. CD_CODIOR = wOrcamento .and. CD_DATAO = M_DATSI
   ...
   If Trim(SD_SAIDA) = "CD IG" .or. Trim(SD_SAIDA) = "CDZ IG"
      ...
      wFil = "20"
   Endif
   If Trim(SD_SAIDA) = "CD JUA" .or. Trim(SD_SAIDA) = "CDZ JUA"
      wSaidaCds = "Juaze"
      wFil = "99"
   Endif
   If Trim(SD_SAIDA) = "CD SOB" .or. Trim(SD_SAIDA) = "CDZ SOB" .or. Trim(SD_SAIDA) = "DEP SOB" .or. Trim(SD_SAIDA) = "DEP. SOB" .or. Trim(SD_SAIDA) = "DEP.SOB"
      wSaidaCds = "Sobral"
      wFil = "11"
   Endif
   If Trim(SD_SAIDA) = "CD FOR" .or. Trim(SD_SAIDA) = "CDZ FOR"
      wSaidaCds = "Fort"
      wFil = "19"
   Endif
   If Trim(SD_SAIDA) = "CD TAUA" .or. Trim(SD_SAIDA) = "CDZ TAUA"
      wSaidaCds = "Taua"
      wFil = "10"
   Endif
 ...
 Enddo
  ...
  If wIdPart >0
     ...
     !echo &wFasePostgres  >> /tmp/orca_&wtesteOrcam
  Else
     wFasePostgres = "Participante N„o Cadastrado"
     !echo &wFasePostgres  >> /tmp/orca_&wtesteOrcam
  Endif
 ...
 //-> Atualiza Dados no Banco
   ...
   If !oOrcamento:insert(cFields,cValues,aParam)
      ...
      wFasePostgres = "Orcamento N„o Inserido "
      !echo &wFasePostgres  >> /tmp/orca_&wtesteOrcam
 ...
 // Itens do Orcamento
    ...
    *SR_StartTrace(SR_GetActiveConnection())
     ...
     If !oItens_de_Orcamento:insert(cFields,cValues,aParam)
        ...
        wIdOrcamento = 0
        wFasePostgres = "Itens de Orcamento N„o Inserido "
        !echo &wFasePostgres  >> /tmp/orca_&wtesteOrcam
     Else
...
//***********************
  ...
  If cBrowseParticipantes->(Reccount()) <= 0 .or. lRet==.F.
     *cMensagem:="N„o ha registros a exibir!"
     *Alert(cMensagem)
...
//************************
  Function Cons_Saldo_oDoo(wCodigo,Pid,wCnPj)      // Consulta Saldos no oDoo
//************************
  Local Pega_Saldo
  Pega_Saldo = "0"

  w_arq := "/tmp/SD_"+Pid+"_"+alltrim(Str(wCodigo))+".txt"
  If File("&w_arq")
     DeleteFile("&w_arq")
  Endif
  telsd=savescreen(19,15,19,78)
  @ 19,15 say "Aguarde, Consultando Saldos no oDoo... "

  wCnpjFilial:=CharOnly("0123456789",wCnPj)

  If File("/opt/odoo_rpc/consulta_saldo.py")
     wEnvia:="python3.6 /opt/odoo_rpc/consulta_saldo.py -c "+wCnpjFilial+" -p "+Alltrim(Str(wCodigo))+" --pid "+Alltrim(Pid)
     !&wEnvia
  Else
     Alert("Arquivo: /opt/odoo_rpc/consulta_saldo.py N„o Encontrado")
  Endif
  ...
  If File("&w_arq")
     ...
     W_Len      := Len(resPid)
     Pega_Saldo := Substr(resPid,1,w_Len)
  Endif
  @ 19,54 say Pega_Saldo

  Inkey(.1)
  RestScreen(19,15,19,78,telsd)

  If File("&w_arq")
     DeleteFile("&w_arq")
  Else
     Pega_Saldo = "0.00"
     Alert("Arquivo de Retorno de Venda N„o Encontrado; ou Sem acesso ao oDoo")
  Endif
Return(Val(Pega_Saldo))

//***********************
  Function SaldooDooGeral(wCod,Pid,Centx)      // Consulta Saldos no oDoo todos os CNPJ
//***********************
  Sele 58
  Veriout(SETCOLOR(),23,"filiais")

  w_arq := "/tmp/SD_"+Pid+"_"+alltrim(Str(wCod))+".csv"
  If File("&w_arq")
     DeleteFile("&w_arq")
  Endif

  telsd=savescreen(00,01,23,78)
  @ 19,15 say "Aguarde, Consultando Saldos no oDoo... "

  If File("/opt/odoo_rpc/consulta_saldo_geral.py")
     wEnvia:="python3.6 /opt/odoo_rpc/consulta_saldo_geral.py -p "+Alltrim(Str(wCod))+" --pid "+Alltrim(Pid)
     !&wEnvia
  Else
     Alert("Arquivo: /opt/odoo_rpc/consulta_saldo_geral.py N„o Encontrado")
  Endif

  Priv wCodigo[200],wCNPJ[200],wSaldo[200],wReservado[200],wDisponivel[200]

  wRet:=EstoqueOdooGeral(w_arq,wCod)    // Busca saldo de todos CNPJ

  RestScreen(00,01,23,78,telsd)
  ...
  If valType(wRet) <> "U"
     ...
     Endif
  Else
     Alert("Sem Conex„o Com o oDoo !")
     Return(0)
  Endif

  m_Ct = 1
  While M_Ct <= Val(wRet)-1
    M_Codx  = wCodigo[m_ct]
    wCGC    = wCNpJ[m_ct]
    wSdo    = wSaldo[m_ct]
    wReserv = wReservado[m_ct]
    wDispon = wDisponivel[m_ct]

    If m_Ct = 1
       wRegional:=DevolveRegional(@listFil)
    Endif

    wComponent:=DevolvePrincipal(m_codx)                // Funcoes.prg
    If wComponent = M_codx
       m_ct++
       Loop
    Endif

    mlj:=DevolveFilial(wCGC)
    If Empty(mlj)
       m_Ct++
       Loop
    ElseIf mlj = "08"
    ...
    ElseIf mlj = "62"
       mlj = "65"
    Endif
    If wCGC = "41426966001306" .and. mlj <> "11"
       mlj = "11"
    ElseIf wCGC = "41426966000334" .and. mlj <> "20"
       mlj = "20"
    ElseIf wCGC = "41426966003945" .and. mlj <> "13"     // CNPJ reaproveitado em outra Filial (33)
       mlj = "13"
    ElseIf wCGC = "41426966001225" .and. mlj <> "70"     // CNPJ reaproveitado em outra Filial (08)
       mlj = "70"
    ElseIf wCGC = "41426966006022" .and. mlj <> "65"     // CNPJ reaproveitado em outra Filial (62)
       mlj = "65"
    Endif
    ...
    Set Order to 1
    Seek m_codx
    If cd_compo >0                // Atualizando sem os componentes
       m_codx = cd_compo
       wCompo = cd_compo
    Endif
    If wCompo >0
    ...
    for k = 1 to len(wRegional)
        If ListFil[k] = mlj
          wLjm = Alltrim(str(val(mlj)))
          If !Eof()
             If CD_CENTL&wLjm <> wDispon
                If RLock()    // Se estiver travado nao atualiza
                   Repl CD_CENTL&wLjm WITH wDispon
                Else
                   @ 23,03 Say "Codigo : "+Strzero(m_codx,6)+" esta Travado !"
                Endif
                Unlock
             Endif
          Endif
...
//*********************
  Function EstoqueOdooGeral(cArquivoCSV,m_codx,wAcao)    // Busca saldo de todos CNPJ
//*********************
Local nPointer := 0
    ...
    For x=1 To 5
       ...
       If cConteudo <> "41426966004321" .or. cConteudo <> "41426966006960"   //  Tiangua 44 desativada e Ze Flex
          ...
          ElseIf wtotCampos  = 2
             wCNPJ[&m]       = cConteudo
          ElseIf wtotCampos  = 3
             wSaldo[&m]      = Val(cConteudo)
          ElseIf wtotCampos  = 4
             wReservado[&m]  = Val(cConteudo)
          ElseIf wtotCampos  = 5
             wDisponivel[&m] = Val(cConteudo)
             M = STRZERO(Val(m)+1)
          Endif
       Endif
    Next
...
If wAcao <> "Sair"
   ConfereCNPjFiliais(cArquivoCSV,m_codx,m)
Endif
...
/*
 ...
 lin = 01
 While m_ct <= val(m)-1
   @ lin,02 say wCodigo[m_ct]
   @ row(),col()+2 say wCnPJ[m_ct]
   @ row(),col()+4 say wDisponivel[m_ct]
   lin++
   m_ct++
 Enddo
 Inkey(0)
...
//*************************
  Function DevolvePrincipal(wCodi)    // Verifica se eh componente e devolve o Principal
//*************************
  Local wRet
  wRet = 0
  Sele 3
...
//**********************
  Function DevolveFilial(wCNpJ)
//**********************
  Local wNrFilial:= ""
  Sele 58
  Go Top
  While !Eof()
    If fili <> "00" .and. fili <> "01" .and. fili <> "08" .and. fili <> "12" .and. Fili <> "15" .and. Fili <> "18" .and. fili <> "22" .and. fili <> "33" .and. Fili <> "44" .and. Fili <> "50" .and. Fili <> "62" .and. Fili <> "64"
       wCGC := Ret_Num(CGC)
       If wCGC = alltrim(wCNpJ)
          wNrFilial = Fili
          Exit
       Endif
    Endif
...
//**************************
 Function ConfereCNPjFiliais(cArquivoCSV,m_codx,m)
//**************************
   Sele 58
   Go Top
   While !Eof()
     If fili <> "00" .and. fili <> "01" .and. fili <> "12" .and. fili <> "08" .and. Fili <> "15" .and. Fili <> "18" .and. fili <> "22" .and. fili <> "33" .and. Fili <> "44" .and. Fili <> "50" .and. Fili <> "62" .and. Fili <> "64"
        wCgc = Ret_num(cgc)
        wRet = .f.
        ConferindoFiliais(wCgc,@wRet,@M)        // Verifica se todas as filiais da regional estao no arquivo
        If wRet = .f.
           Setcolor("n+/w")
           Set Devi to Print
           Set Print to &cArquivoCSV ADDITIVE
           @ PROW()+00,00 Say Alltrim(str(m_codx))+","
           @ PROW()+00,PCOL() Say wCgc+",0,0,0"+chr(10)
           Set Devi to Screen
           @ 18,00 say ""
           Set Print to
           Run NOTEPAD &cArquivoCSV
         Endif
     Endif
     Skip
   Enddo

   EstoqueOdooGeral(cArquivoCSV,m_codx,"Sair")    // Busca saldo de todos CNPJ

Return

//**************************
  Function ConferindoFiliais(wCgc,wRet,m)        // Verifica se todas as filiais da regional estao no arquivo
//**************************
 Clear
 m_ct = 1
 lin = 01
 While m_ct <= val(m)-1
   If alltrim(wCnPJ[m_ct]) = alltrim(wCgc)
      wRet:= .t.
      Exit
   Endif
   m_ct++
...
//**********************
  Function DevolveRegional(ListFil)
//**********************
  Sele 58
  Go top
  While !Eof()
...
//**********************
* Function DevolveRegional(I_Fili,ListFil)
//**********************
 /*
  If I_Fili = "02" .or. I_Fili = "28"
     ListFil:= {"02","28","11","19","20","99"}
  ElseIf I_Fili = "06" .or. I_Fili = "09" .or. I_Fili = "32" .or. I_Fili = "54"
     ListFil:= {"06","09","32","54","11","19","20","99"}
  ElseIf I_Fili = "16" .or. I_Fili = "17" .or. I_Fili = "23" .or. I_Fili = "65" .or. I_Fili = "72"
     ListFil:= {"16","17","23","65","72","11","19","20","99"}
  ElseIf I_Fili = "25" .or. I_Fili = "45"
     ListFil:= {"25","45","53","11","19","20","99"}
  ElseIf I_Fili = "37" .or. I_Fili = "39"
     ListFil:= {"37","39","10","11","19","20","99"}
  ElseIf I_Fili = "41" .or. I_Fili = "47"
     ListFil:= {"41","47","11","19","20","99"}
  ElseIf I_Fili = "53"
     ListFil:= {"53","25","45","56","57","58","66","11","19","20","99"}
  ElseIf I_Fili = "56" .or. I_Fili = "57" .or. I_Fili = "58" .or. I_Fili = "66"
     ListFil:= {"56","57","58","66","11","19","20","99"}
  ElseIf I_Fili = "67" .or. I_Fili = "71" .or. I_Fili = "73"
     ListFil:= {"67","71","73","11","19","20","99"}
  ElseIf I_Fili = "38" .or. I_Fili = "42" .or. I_Fili = "43"
     ListFil:= {"38","42","43","11","19","20","99"}
  ElseIf I_Fili = "31" .or. I_Fili = "48" .or. I_Fili = "63"
     ListFil:= {"31","48","63","11","19","20","99"}
  ElseIf I_Fili = "14" .or. I_Fili = "34" .or. I_Fili = "49" .or. I_Fili = "70"
     ListFil:= {"14","34","49","70","11","19","20","99"}
  ElseIf I_Fili = "10" .or. I_Fili = "11" .or. I_Fili = "19" .or. I_Fili = "20" .or. I_Fili = "99"
     Sele 58
     Go Top
     While !Eof()
       If fili <> "00" .and. fili <> "01" .and. fili <> "08" .and. fili <> "12" .and. Fili <> "15" .and. Fili <> "18" .and. fili <> "22" .and. fili <> "33" .and. Fili <> "44" .and. Fili <> "50" .and. Fili <> "62" .and. Fili <> "64"
...
//****************
  Procedure csvcclientesativosInativos()
//****************
 // Cabecalho do Arquivo
 Clear
 Stor 0 to TtotalAtivo,TtotalInativo,Ttotal

 m_arqs = "/zenir/sistemas/tabela/clientesativosInativos.csv"
 If file(m_arqs)
    DeleteFile(m_arqs)
 Endif

 nHand :=FCreate("&m_arqs",777)
 wSep = "|"

 cLinha:=("Filial         "+wSep+"Clientes Ativos/Inativos Carne")
 fWrite(nHand,cLinha+chr(13)+chr(10))
 cLinha:=(" ")
 FWrite(nHand,cLinha+chr(13)+chr(10))
 ...
 Go Top
 While !Eof()
   If CD = "S" .or. Fili = "00" .or. Fili = "01" .or. Fili = "12" .or. Fili = "18" .or. Fili = "22"
      Skip
      Loop
   Endif
   wFilial = M_LojaD

   wPath = "/zenir/sistemas/loja"+Fili+"/"
   chdir(wPath)
   @ 08,30 say curdir()

   Stor 0 to wtotalAtivo,wtotalInativo,wtotal,tComprou

   wIndex = wPath+"index_clientes"
   wDbf  = wPath+"cdclie.dbf"

   Sele 1
   Veriout(SETCOLOR(),23,"&wDbf")
   Set index to &wIndex

   wIndex = wPath+"index_cdsai2"
   wDbf  = wPath+"cdsai2.dbf"
   ...
   While !Eof()
     ...
     wComprow:=RetUltimosDozem(wCliente,m_datsi-365)
     If wComprow = .t.
        tComprou++
     Endif
   ...
   wLen=Len(wFilial)
   cLinha:=(wFilial+wSep+"Clientes Ativos (Carne)   "+Str(wTotalAtivo)+wSep+" "+Str((wtotalAtivo/wtotal)*100)+" %")
   FWrite(nHand,cLinha+chr(13)+chr(10))

   cLinha:=(space(wLen)+wSep+"         Inativos         "+Str(wTotalInativo)+wSep+" "+Str((wtotalInativo/wtotal)*100)+" %")
   FWrite(nHand,cLinha+chr(13)+chr(10))

   cLinha:=(space(wLen)+wSep+"Comprou Ultimos 12 Meses  "+Str(tComprou)+wSep+" "+Str((tComprou/wtotal)*100)+" %")
   FWrite(nHand,cLinha+chr(13)+chr(10))

   cLinha:=("Total de Clientes Cadastrados             "+Str(wTotal))
   FWrite(nHand,cLinha+chr(13)+chr(10))
   cLinha:=(" ")
   FWrite(nHand,cLinha+chr(13)+chr(10))
 ...
 Enddo
   cLinha:=("--------------------------------------------")
   FWrite(nHand,cLinha+chr(13)+chr(10))
   cLinha:=("Total Geral Ativos         "+Str(TtotalAtivo))
   FWrite(nHand,cLinha+chr(13)+chr(10))
   cLinha:=("Total Geral Inativos       "+Str(TtotalInativo))
   FWrite(nHand,cLinha+chr(13)+chr(10))
   cLinha:=("Total Geral                "+Str(Ttotal))
   FWrite(nHand,cLinha+chr(13)+chr(10))
 FClose(nHand)
Return

//****************
  function RetUltimosDozem(wCliente,wData)
//****************
 Local wRet:= .f.
 Sele 4
 Set Orde to 14
 Go top
 While .t.
   Seek str(wCliente,6,0)+substr(dtoc(wData),7,4)+substr(dtoc(wData),4,2)+substr(dtoc(wData),1,2)
   If Eof()
      wData++
      If wData > m_datsi
         Exit
